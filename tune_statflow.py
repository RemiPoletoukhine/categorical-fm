from train_statflow import train_epoch, validate_epoch, load_qm9
...
def hyperparam_tuning(
    ...
    if os.name == 'nt':
        log_dir = datetime.now().replace(':', '')
    else:
        log_dir = datetime.now()

    study = optuna.create_study(
        direction="minimize",
        storage=f"sqlite:///optuna_logs/Trial_{log_dir}.sqlite3",
        study_name="StatFlow",
    )
