...
class EigenFeatures:
    ...
    def __call__(self, noisy_data):
        ...
        mask_diag = mask_diag * (~mask.unsqueeze(1).type(torch.bool)) * (~mask.unsqueeze(2).type(torch.bool))
...
def get_eigenvectors_features(vectors, node_mask, n_connected, k=2):
    ...
    random = torch.randn(bs, n, device=node_mask.device) * (~node_mask.type(torch.bool))                                   # bs, n
